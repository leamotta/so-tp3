...
class Node(object):
        ...
        while self.__comm.iprobe() > 0 and cantMensajes > 0:
            ...
            for node_recieved in recieved_node_list:
                if not node_recieved in processed:
                    # sumo a nodes_min
                    nodes_min[node_recieved[0]] = node_recieved[1]; 
                    # Como ya sabemos que no esta en los procesados, lo agrego a esa cola
    ...
    def __find_nodes_join(self, contact_nodes):
        ...
        while self.__comm.iprobe() > 0 and cantMensajes > 0:        
            ...
            # Por cada nodo que llegó en la última iteración, vuelvo a revisar si está entre los procesados y envío el mensaje. 
            for node in queue:
                if node[1] not in processed:
                    processed.add(node[1])
                    # llamo no bloqueante para tener los valores mas cercanos
